...
export async function canViewModule(moduleKey: string): Promise<boolean> {
  try {
    ...
    // supabase.auth.* puo' lanciare (non solo restituire { error }) su
    // problemi di rete/token, a differenza delle query .from().select().
    // Senza questo catch, una PWA con token scaduto o rete instabile al
    // risveglio resta bloccata su una pagina bianca: chi chiama questa
    // funzione tratta "false" come "nega l'accesso e reindirizza",
    // invece di restare in attesa di una promise che non si risolve mai.
    console.error("Errore rete/sessione durante verifica permesso modulo:", err);
    return false;
  }
}
...
export async function getUserLevelRank(): Promise<number | null> {
  try {
    ...
    console.error("Errore rete/sessione durante verifica rank utente:", err);
    return null;
  }
}
